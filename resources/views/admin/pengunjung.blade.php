...
<div class="container-fluid">
    <div class="card card-documentation">
        ...
        <div class="card mt-4">
            <div class="card-body">
            <button class="btn btn-info" data-toggle="modal" data-target="#addPengunjungModal">Tambah Pengunjung</button>
                    ...
                    <tbody>
                        @foreach($users as $key => $user)
                            <tr>
                                ...
                                <td>
                                    <form action="{{ route('admin.pengunjung.destroy', $user->id) }}" method="POST" style="display: inline-block;">
                                        @csrf
                                        @method('DELETE')
        ...
        <div class="modal fade" id="editPengunjungModal" tabindex="-1" role="dialog" aria-labelledby="editPengunjungModalLabel" aria-hidden="true">
            <div class="modal-dialog" role="document">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="editPengunjungModalLabel">Edit Pengunjung</h5>
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                            <span aria-hidden="true">&times;</span>
                        </button>
                    </div>
    ...
    function showEditForm(id, name, email, role, phone_number) {
        // Mengisi nilai form edit dengan data user
        document.getElementById('edit_name').value = name;
        document.getElementById('edit_email').value = email;
        document.getElementById('edit_role').value = role;
        document.getElementById('edit_phone_number').value = phone_number;

        // Mengatur action form agar sesuai dengan route update
        document.getElementById('editPengunjungForm').action = '{{ route('admin.pengunjung.updatez', '') }}/' + id;

        // Menampilkan modal edit
        $('#editPengunjungModal').modal('show');
    }
    </script>
@endsection
